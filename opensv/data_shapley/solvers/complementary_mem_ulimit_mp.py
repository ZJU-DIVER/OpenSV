...
def subtask(
    ...
    with tqdm(total=res) as pbar:
        while used_utility < res:
            rng.shuffle(idxes)
            i = rng.choice(N + 1, 1)[0]
            acc_l, used_l = get_utility_memorized(
                mem, x_train, y_train, idxes[:i], x_valid, y_valid, clf
            )
            acc_r, used_r = get_utility_memorized(
                mem, x_train, y_train, idxes[i:], x_valid, y_valid, clf
            )
            if used_l:
                used_utility += 1
...
def complementary_mem_ulimit_mp(
    x_train: Array,
    y_train: Array,
    x_valid: Optional[Array] = None,
    y_valid: Optional[Array] = None,
    ...
    with open("log.txt", "a") as f:
        f.write(f"complementary_mem_ulimit_mp,{N},{num_utility},{mem._get(-1)}\n")
