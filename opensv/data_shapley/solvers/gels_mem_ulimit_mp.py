...
def subtask(
    ...
    with tqdm(total=num_utility) as pbar:
        while num_utility > 0:
            ...
            rng.shuffle(idxes)
            u, used = get_utility_memorized(
                mem, x_train, y_train, idxes[:s], x_valid, y_valid, clf
            )
            if used:
                num_utility -= 1
                pbar.update(1)
...
def gels_mem_ulimit_mp(
    ...
    pool = Pool()
    func = partial(subtask, mem, x_train, y_train, x_valid, y_valid, clf, q)
    ...
    with open("log.txt", "a") as f:
        f.write(f"gels_mem_ulimit_mp,{N},{num_utility},{mem._get(-1)}\n")
